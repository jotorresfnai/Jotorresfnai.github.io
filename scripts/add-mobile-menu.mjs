...
@media(max-width:760px){
  ...
  .navigation{position:absolute!important;top:100%!important;left:0!important;right:0!important;width:100%!important;display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:0!important;padding:8px 5% 14px!important;background:#07171b!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important;transform:translateY(-6px);transition:opacity .2s ease,transform .2s ease,visibility .2s ease;box-shadow:0 14px 28px rgba(0,0,0,.25)}
  ...
  .navigation a{padding:10px 5px;border-bottom:1px solid rgba(255,255,255,.1);font-size:15px!important;line-height:1.25}
  .navigation .whatsapp{margin-top:10px;text-align:center}
}
`;
...
function ensureMenuLinks(html) {
  ...
  const finalWhatsapp = whatsappHtml || '<a class="whatsapp" href="https://wa.me/41798362510" target="_blank" rel="noopener">WhatsApp</a>';
  const links = MENU_LINKS.replace(/<a class="whatsapp"[\s\S]*?<\/a>/i, finalWhatsapp);
  const openTag = nav.match(/^<nav\b[^>]*>/i)[0];
  return html.replace(nav, `${openTag}${links}</nav>`);
}
...
for (const file of files) {
  ...
  if (!/\bid=["']navigation["']/i.test(navTag)) html = html.replace(navTag, navTag.replace(/<nav\b/i, '<nav id="navigation"'));
  ...
  html = html.replace(/\s*<style id="site-mobile-menu-css">[\s\S]*?<\/style>/i, '');
  html = html.replace('</head>', `<style id="site-mobile-menu-css">${CSS}</style>\n</head>`);
  html = html.replace(/\s*<script id="site-mobile-menu">[\s\S]*?<\/script>/i, '');
