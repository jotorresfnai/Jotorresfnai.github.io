...
function transformFile(filePath) {
  ...
  if (original.includes("SLIDESHOW_DOS_IMOVEIS_CSS")) return false;

  const match = original.match(/<div\s+class=["']gallery["'][^>]*>[\s\S]*?<\/div>/i);
  if (!match) return false;

  const galleryHtml = match[0];
  const imageCount = (galleryHtml.match(/<img\b[^>]*>/gi) || []).length;
  if (imageCount <= 1) return false;

  const slider = buildSlider(galleryHtml);
  if (!slider) return false;

  const html = addAssets(original.replace(galleryHtml, slider));
