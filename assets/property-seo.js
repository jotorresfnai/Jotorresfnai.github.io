...
(function(){
  ...
  function addJsonLd(property, photos){
    ...
    var old=document.getElementById('propertyJsonLd'); if(old) old.remove();
    var script=document.createElement('script'); script.type='application/ld+json'; script.id='propertyJsonLd'; script.textContent=JSON.stringify(data); document.head.appendChild(script);
  }
  window.applyPropertySeo=function(property, photos){
    ...
    var canonical=document.getElementById('canonical'); if(canonical) canonical.setAttribute('href',url);
  ...
  function ensureCard(){
    ...
    var locationBlocks=card.querySelectorAll('.location');
    locationBlocks.forEach(function(el){ el.style.display='none'; });
    var old=card.querySelector('.card-intro');
    if(!old){
      old=document.createElement('div');
      old.className='card-intro';
      old.textContent='Gostou deste imóvel?';
      old.style.cssText='font-size:17px;font-weight:800;line-height:1.35;color:#07171b;margin:0 0 18px;';
      card.insertBefore(old,card.firstChild);
    }
    var paragraphs=card.querySelectorAll('p');
    paragraphs.forEach(function(p){
      if(!p.closest('.card-intro') && !p.classList.contains('keep-card-text')) p.style.display='none';
    });
    var btn=card.querySelector('.contact-btn');
    if(btn){
      var textNode=btn.querySelector('span');
      if(textNode) textNode.textContent='Fale comigo';
    }
  }
  ...
  function ensureGalleryThumbnails(){
    ...
    var thumbs=gallery.querySelector('.gallery-thumbnails');
    if(!thumbs){
      thumbs=document.createElement('div');
      thumbs.className='gallery-thumbnails';
      thumbs.setAttribute('aria-label','Miniaturas');
      slides.forEach(function(slide,index){
        var source=slide.querySelector('img');
        if(!source) return;
        var b=document.createElement('button');
        b.type='button'; b.className='gallery-thumb'; b.dataset.thumb=String(index); b.setAttribute('aria-label','Fotografia '+(index+1));
        var im=document.createElement('img'); im.src=source.currentSrc || source.src; im.alt='';
        b.appendChild(im); thumbs.appendChild(b);
      });
      gallery.appendChild(thumbs);
    }
    var styleId='property-gallery-fix-style';
    if(!document.getElementById(styleId)){
      var style=document.createElement('style'); style.id=styleId;
      style.textContent='.gallery-thumbnails{position:absolute;left:18px;right:18px;bottom:18px;z-index:35;display:flex;gap:8px;justify-content:center;overflow-x:auto;padding:8px;border-radius:12px;background:rgba(7,23,27,.45);backdrop-filter:blur(4px)}.gallery-thumb{width:72px;height:52px;flex:0 0 72px;padding:0;border:2px solid transparent;border-radius:7px;overflow:hidden;background:#07171b;cursor:pointer;opacity:.8}.gallery-thumb.active{border-color:#c9a34a;opacity:1}.gallery-thumb img{width:100%;height:100%;object-fit:cover;display:block}@media(max-width:760px){.gallery-thumbnails{left:8px;right:8px;bottom:8px;justify-content:flex-start}.gallery-thumb{width:58px;height:44px;flex-basis:58px}}';
      document.head.appendChild(style);
    }
    var buttons=Array.prototype.slice.call(thumbs.querySelectorAll('.gallery-thumb'));
    buttons.forEach(function(btn){
      if(btn.dataset.bound==='1') return;
      btn.dataset.bound='1';
      btn.addEventListener('click',function(){
        var next=Number(btn.dataset.thumb);
        var nextSlide=slides[next];
        if(!nextSlide) return;
        slides.forEach(function(s,i){ s.classList.toggle('active',i===next); s.style.display=i===next?'block':''; });
        buttons.forEach(function(b,i){ b.classList.toggle('active',i===next); });
        var activeImg=nextSlide.querySelector('img');
        if(activeImg) activeImg.scrollIntoView({block:'nearest',inline:'nearest'});
      });
    });
    var activeIndex=slides.findIndex(function(s){ return s.classList.contains('active') || s.style.display==='block'; });
