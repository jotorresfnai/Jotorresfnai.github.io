...
function show(n,user){index=(n+slides.length)%slides.length;slides.forEach((s,i)=>s.classList.toggle('is-active',i===index));thumbs.forEach((t,i)=>t.classList.toggle('is-active',i===index));if(counter)counter.textContent=(index+1)+' / '+slides.length;const a=thumbs[index];if(a&&strip)strip.scrollTo({left:Math.max(0,a.offsetLeft-strip.clientWidth/2+a.offsetWidth/2),behavior:'smooth'});if(user)restart()}
...
function transformFile(file){
  ...
  const slides=images.map((x,i)=>`<div class="gallery-slide${i===0?' is-active':''}"><img src="${x.src}" alt="${x.alt.replace(/"/g,'&quot;')}"></div>`).join('\n');
  const thumbs=images.map((x,i)=>`<button type="button" class="gallery-thumb${i===0?' is-active':''}" aria-label="Ver fotografia ${i+1}"><img src="${x.src}" alt="${x.alt.replace(/"/g,'&quot;')}"></button>`).join('\n');
...
if(fs.existsSync(ROOT)){
 let changed=0;
 for(const dir of fs.readdirSync(ROOT,{withFileTypes:true}).filter(x=>x.isDirectory())){const file=path.join(ROOT,dir.name,'index.html');if(fs.existsSync(file)){if(transformFile(file))changed++;console.log(`Galeria verificada: ${file}`)}}
